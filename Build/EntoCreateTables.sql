...
BEGIN TRY
	...
	CREATE TABLE dbo.ExpectedTypes (
		...
		CONSTRAINT FK_TaxonID1 FOREIGN KEY (TaxonID) REFERENCES Taxons(TaxonID) ON DELETE CASCADE,
		PRIMARY KEY (TypeID)
	)
	...
	CREATE TABLE dbo.ExpectedStages (
		...
		TaxonID int NOT NULL,
		CONSTRAINT FK_TaxonID2 FOREIGN KEY (TaxonID) REFERENCES Taxons(TaxonID) ON DELETE CASCADE,
		PRIMARY KEY (StageID)
	)
